...
from knnbox.combiner.mu_calculator import MuCalculator
...
class AdaptiveCombiner(nn.Module):
    ...
    def __init__(self,
        ...
        self.mu_calculator = MuCalculator(self.max_k, use_context_dis=True)

    def get_knn_prob(self, vals, distances, device="cuda:0", cache_hidden=None, source_weight=0.0, **kwargs):
        ...
        scale = self.mu_calculator.meta_k_network(vals, distances, context_dist)["mu_net_output"] * source_weight
        context_dist = context_dist * scale
        tmp_distances = distances + context_dist
        distances = tmp_distances

        metak_outputs = self.meta_k_network(vals, distances, context_dist)
    ...
    def __init__(
        ...
        if k_trainable:
            self.distance_to_k = nn.Sequential(
                nn.Linear(self.max_k * 3 if self.label_count_as_feature else self.max_k, k_net_hid_size),
                nn.Tanh(),
            ...
            if self.label_count_as_feature:
                nn.init.xavier_normal_(self.distance_to_k[0].weight[:, :self.max_k], gain=0.1)
                nn.init.xavier_normal_(self.distance_to_k[0].weight[:, self.max_k: 2 * self.max_k], gain=0.1)
                nn.init.xavier_normal_(self.distance_to_k[0].weight[:, 2 * self.max_k:], gain=0.1)
            else:
        ...
        if lambda_trainable:
            self.distance_to_lambda = nn.Sequential(
                ...
                nn.Linear(k_net_hid_size, 1),
                nn.Sigmoid()
            )

            if self.label_count_as_feature:
                nn.init.xavier_normal_(self.distance_to_lambda[0].weight[:, :self.max_k], gain=0.1)
                nn.init.xavier_normal_(self.distance_to_lambda[0].weight[:, self.max_k: 2 * self.max_k], gain=0.1)
                nn.init.xavier_normal_(self.distance_to_lambda[0].weight[:, 2 * self.max_k:], gain=0.1)
            else:
