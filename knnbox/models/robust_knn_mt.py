...
class RobustKNNMTDecoder(TransformerDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if args.knn_mode == "build_datastore":
            ...
            self.datastore = Datastore.load(args.knn_datastore_path, load_list=["keys", "vals", "hiddens_idx", "hiddens", "keys"])
    ...
    def forward(
        ...
        # not use robust knn-mt to build datastore, use vanilla knn-mt
        ...
        if self.args.knn_mode == "build_datastore":
            ...
            self.retriever.fast_faiss_retrieve_with_hidden(x, self.cache_hidden, self.hidden_dic, self.gen_len, self.args.knn_mode, return_list=["keys", "vals", "distances", "hiddens_idx", "hiddens", "indices", "query"])
    ...
    def get_normalized_probs(
        ...
        if self.args.knn_mode == "inference" or self.args.knn_mode == "train_metak":
            ...
            self.retriever.results["hiddens"] = self.retriever.results["hiddens"].float()

            knn_dists = torch.sqrt(torch.sum((knn_key - queries.unsqueeze(-2).detach()) ** 2, dim=-1))

            hiddens = self.retriever.results["hiddens"]
            context_dist, knn_dists = self.combiner.mu_calculator.robust_calculate_new_distance(tgt_index, knn_dists, cache_hidden=self.cache_hidden, hiddens=hiddens)

            knn_dists, new_index = torch.sort(knn_dists, dim=-1)
            context_dist = context_dist.gather(dim=-1, index=new_index)
            ...
            if self.training:
                ...
                with torch.no_grad():
                    ...
                    noise_context_dist = torch.cat([torch.zeros(B, S, 1).cuda(), context_dist[:, :, :-1]], -1)
                    ...
                    real_context_dist = noise_context_dist * rand_mask2.long() + context_dist * (1 - rand_mask2.long())
                    ...
                    noise_knn_dists = torch.sqrt(torch.sum((knn_key - last_hidden.unsqueeze(-2).detach()) ** 2, dim=3))
                    noise_knn_dists = real_context_dist + noise_knn_dists
